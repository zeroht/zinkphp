 ...
 *
 *  // GET
 *  $rs = $curl->get('http://www.baidu.com');
 *
 *  // POST
 *  $data = array('username' => 'limHu');
 *  $rs = $curl->post('http://www.baidu.com', $data);
```
 *
 * @author :  HuYang
 * @version: 1.1.0
...
class CUrl
{
    ...
    protected function request($url, $data, $timeoutMs=3000)
    {
        ...
        $curRetryTime = $this->retryTimes;
        do {
            $rs = curl_exec($ch);
            $curRetryTime--;
        } while($rs == FALSE && $curRetryTime >= 0);
